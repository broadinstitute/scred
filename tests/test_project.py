...
def test_RedcapProject_init_with_token_and_url():
    ...
    rp = RedcapProject(token=faketoken, url=fakeurl)


def test_cbnames_returns_exports_for_existing_field(
    stored_metadata_json, stored_efn_json
):
    faketoken = "ABCD9999DDDDXXZZ067JTP01Y44MSPD1"
    fakeurl = "https://redcap.botulism.org/api/"
    rp = RedcapProject(token=faketoken, url=fakeurl)
...
def test_any_endorsed_returns_False_if_none_endorsed(
    stored_RecordSet_instance, stored_metadata_json, stored_efn_json
):
    faketoken = "ABCD9999DDDDXXZZ067JTP01Y44MSPD1"
    fakeurl = "https://redcap.botulism.org/api/"
    rp = RedcapProject(token=faketoken, url=fakeurl)
